...
public class EverythingBagelPortlet extends QBiCPortletUI {

    private static final Logger LOG = LogManager.getLogger(EverythingBagelPortlet.class);

    @Override
    protected Layout getPortletContent(final VaadinRequest request) {
        LOG.info("Generating content for {}", EverythingBagelPortlet.class);
        
        final StringBuilder builder = new StringBuilder("<h1><b>This is a sample portlet.</b></h1>");
        if (PortalUtils.isLiferayPortlet()) {
            ...
            builder.append("This portal has ").append(getPortalCountOfRegisteredUsers()).append(" registered users (according to the data returned by Liferay's API call)<br/>");
        } else {
            builder.append("You are currently in a local testing mode. No Liferay Portlet context found.<br/>");
        }

        builder.append("Since this portlet grew from a simple 'Hello World' to a 'test all things', the name 'everything bagel' seemed somehow appropriate.");
        ...
        insertTestComponents(panelContent);

        final Panel mainPanel = new Panel("Everything Bagel Portlet");
    ...
    private void insertTestComponents(final Layout layout) {
    ...
    private void insertOpenBISConnectionTesterComponents(final Layout layout) {
        ...
        openBisTestButton.addClickListener((Button.ClickListener) event -> {
            if (isOpenBISWorking(dataSourceUser.getValue(), dataSourcePassword.getValue(), dataSourceUrl.getValue())) {
                Styles.notification("Everything Bagel Portlet", "Connection to openBIS works!", NotificationType.SUCCESS); 
            } else {
                Styles.notification("Everything Bagel Portlet", "Could not establish openBIS connection. Check the logs for exceptions.", NotificationType.ERROR); 
            }
        });
